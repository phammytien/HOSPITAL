...
@section('content')
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <p class="text-gray-600 mt-1">Quản lý danh sách thuốc, vật tư y tế và các loại hàng hóa khác trong bệnh
                    viện.</p>
            </div>
            <div class="flex space-x-3">
                ...
                <button onclick="document.getElementById('importExcelInput').click()"
                    class="border border-gray-300 bg-white hover:bg-gray-50 text-gray-700 px-4 py-2 rounded-lg font-medium">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <p class="text-xs text-gray-500 mt-2">{{ $allProducts->unique('category_id')->count() }} nhóm danh mục con
                </p>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <h3 class="text-3xl font-bold text-gray-800">
                    {{ $allProducts->where('created_at', '>=', now()->subDays(30))->count() }}
                </h3>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <h3 class="text-3xl font-bold text-gray-800">{{ $allProducts->where('stock_quantity', '<', 10)->count() }}
                </h3>
            ...
            <div class="w-64 flex-shrink-0">
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button onclick="openAddModal()"
                            class="text-blue-600 text-sm cursor-pointer hover:underline uppercase font-bold">Thêm
                            mới</button>
                    </div>

                    <div class="space-y-1">
                        <a href="{{ route('admin.categories') }}"
                            class="flex items-center justify-between px-3 py-2 rounded-lg {{ !request('category_id') ? 'bg-blue-50 text-blue-600 font-bold' : 'text-gray-700 hover:bg-gray-50' }}">
                        ...
                        @foreach($allCategories as $cat)
                            <a href="{{ route('admin.products.index', ['category_id' => $cat->id]) }}"
                                ...
                                <div class="flex items-center">
                                    <i
                                        class="fas fa-folder w-5 {{ request('category_id') == $cat->id ? 'text-blue-500' : 'text-gray-400 group-hover:text-blue-500' }}"></i>
                                    <span class="ml-2 truncate max-w-[120px]"
                                        title="{{ $cat->category_name }}">{{ $cat->category_name }}</span>
                                </div>
                                <span
                                    class="text-xs {{ request('category_id') == $cat->id ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600' }} px-2 py-0.5 rounded-full">{{ $cat->products_count ?? 0 }}</span>
                            </a>
                        @endforeach
                ...
                <div class="p-6 border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="flex-1 max-w-md">
                            <div class="relative">
                                <input type="text" id="searchInput" placeholder="Tìm kiếm tên thuốc, mã SKU..."
                        ...
                        <div class="flex items-center space-x-3 ml-4">
                            <select id="statusFilter" onchange="filterProducts()"
                                ...
                                <option value="all" {{ request('status') == 'all' ? 'selected' : '' }}>Trạng thái</option>
                                <option value="available" {{ request('status') == 'available' ? 'selected' : '' }}>Còn hàng
                                </option>
                                <option value="out" {{ request('status') == 'out' ? 'selected' : '' }}>Hết hàng</option>
                            ...
                            @forelse($categories as $category)
                                ...
                                <tr class="hover:bg-gray-50 transition cursor-pointer"
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <i id="chevron-{{ $category->id }}"
                                            class="fas fa-chevron-right text-gray-400 transition-transform duration-200"></i>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-sm space-x-2" onclick="event.stopPropagation()">
                                        <button
                                            onclick="openEditModal({{ $category->id }}, '{{ addslashes($category->category_name) }}', '{{ addslashes($category->description ?? '') }}', '{{ addslashes($category->category_code ?? '') }}')"
        ...
        @if(isset($filteredProducts) && $filteredProducts->isNotEmpty())
            <div class="mt-8">
                <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                    ...
                    Sản phẩm thuộc danh mục: <span
                        class="text-blue-600 ml-1">{{ $categories->first()->category_name ?? '' }}</span>
                    <span
                        class="ml-3 px-2 py-0.5 rounded-full bg-blue-100 text-blue-600 text-xs font-bold">{{ $filteredProducts->count() }}
                        sản phẩm</span>
                </h3>
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        @else
                                            <span
                                                ...
                                                <span class="w-1.5 h-1.5 bg-green-500 rounded-full mr-1.5"></span>Còn hàng
                                                ({{ $prod->stock_quantity }})
                                            </span>
                                        @endif
                                    </td>
    ...
    @push('scripts')
        <script>
            ...
            function toggleProducts(categoryId) {
                ...
                // Check if we are expanding a new row
                const isExpanding = productsRow.classList.contains('hidden');

                // Close all currently expanded rows
                expandedCategories.forEach(id => {
                    if (id !== categoryId) { // Optional check, but good for safety
                ...
                // Clear the set and only re-add the current one if it's being expanded
                ...
                if (isExpanding) {
                    ...
                    if (!contentDiv.dataset.loaded) {
                        ...
                        fetch(`/admin/categories/${categoryId}/products?status=${status}`)
                            .then(res => res.json())
                            .then(data => {
                                if (data.success) {
                    ...
                    // Collapse (already handled by classList logic above logic, but explicitly purely for clarity: we just don't add it back to expandedCategories)
            ...
            function renderProducts(categoryId, products) {
                ...
                products.forEach(product => {
                    ...
                    let stockBadge = '<span class="px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">Sẵn hàng</span>';

                    if (product.stock_quantity <= 0) {
                        status = 'out';
                        stockBadge = '<span class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">Hết hàng</span>';
                    } else if (product.stock_quantity <= 10) {
                        status = 'low';
                        stockBadge = '<span class="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">Sắp hết</span>';
                    }

                    html += `
                                <div class="product-card bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition" data-status="${status}">
            ...
            function filterProducts() {
                ...
                // Keep category_id if exists (for maintaining expanded state or other logic if needed, though here we filter list)
                // Actually, if we filter list, we might lose pagination/context, but that's expected.
