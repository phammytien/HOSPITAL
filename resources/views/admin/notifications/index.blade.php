...
@section('content')
    ...
    {{-- Success Modal --}}
    @if(session('success'))
    <div id="toast" class="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-gray-900 bg-opacity-50 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full text-center transform transition-all scale-100">
            <div class="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-green-100 mb-6">
                <i class="fas fa-check text-green-600 text-4xl"></i>
            </div>
            <h3 class="text-2xl font-bold text-gray-900 mb-2">Thành công!</h3>
            <p class="text-gray-600 mb-6 text-lg">{{ session('success') }}</p>
            <button onclick="closeToast()" class="w-full py-3 px-4 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-75 transition-colors">
                Đóng
            </button>
        </div>
    </div>
    ...
    <div id="toast" class="fixed inset-0 z-[100] flex items-center justify-center px-4 bg-gray-900 bg-opacity-50 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-xl shadow-2xl p-8 max-w-md w-full text-center transform transition-all scale-100">
            <div class="mx-auto flex items-center justify-center h-20 w-20 rounded-full bg-red-100 mb-6">
                <i class="fas fa-exclamation text-red-600 text-4xl"></i>
            </div>
            <h3 class="text-2xl font-bold text-gray-900 mb-2">Lỗi!</h3>
            <p class="text-gray-600 mb-6 text-lg">{{ session('error') }}</p>
            <button onclick="closeToast()" class="w-full py-3 px-4 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-75 transition-colors">
                Đóng
            </button>
        </div>
    </div>
                    ...
                    <div class="p-6 hover:bg-gray-50 transition border-l-4 border-{{ $config['color'] }}-500 cursor-pointer {{ !$notification->is_read ? 'bg-blue-50' : '' }}"
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex items-center gap-2" onclick="event.stopPropagation()">
                                <button
                                    onclick="openEditModal({{ $notification->id }}, '{{ addslashes($notification->title) }}', '{{ addslashes($notification->message) }}', '{{ $notification->type }}', '{{ $notification->target_role }}')"
