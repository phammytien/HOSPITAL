...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl p-6 border border-gray-200">
        ...
        <form action="{{ route('admin.orders.update-status', $order->id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            @csrf
            <div>
            ...
            <div class="bg-white rounded-xl p-6 border border-gray-200">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-900">{{ $order->order_date ? $order->order_date->format('d/m/Y') : 'N/A' }}</p>
                    </div>
                    <!-- <div>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="grid grid-cols-3 gap-4 text-sm">
                                    ...
                                    <div>
                                        ...
                                        <p class="font-semibold text-gray-900">{{ number_format($item->unit_price, 0, ',', '.') }} đ</p>
                                    </div>
                                    <div>
                                        <p class="text-gray-500 mb-1">Thành tiền</p>
                                        <p class="font-bold text-blue-600">{{ number_format($item->quantity * $item->unit_price, 0, ',', '.') }} đ</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="pt-3 border-t border-gray-200">
                        ...
                        @elseif($order->status == 'APPROVED')
                            <span class="px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs font-semibold">Đã duyệt</span>
                        @elseif($order->status == 'PROCESSING')
                            <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold">Đang xử lý</span>
                        @elseif($order->status == 'DELIVERED')
                            <span class="px-3 py-1 bg-teal-100 text-teal-700 rounded-full text-xs font-semibold">Đã giao hàng</span>
                        @elseif($order->status == 'PAID')
                            <span class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-semibold">Đã thanh toán</span>
                        @elseif($order->status == 'COMPLETED')
                            <span class="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-semibold">Hoàn thành</span>
                        @else
