...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="flex border-b border-gray-200">
            <a href="{{ route('buyer.notifications.index', ['tab' => 'received']) }}" id="tab-received" 
                ...
                Thông báo nhận được
                <span class="ml-2 px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs">{{ $stats['received_total'] }}</span>
            </a>
            <a href="{{ route('buyer.notifications.index', ['tab' => 'sent']) }}" id="tab-sent" 
                ...
                Thông báo đã gửi
                <span class="ml-2 px-2 py-1 bg-blue-100 text-blue-700 rounded-full text-xs">{{ $stats['sent_total'] }}</span>
            </a>
            ...
            <div class="bg-white rounded-xl p-6 border border-gray-200">
                <div class="flex items-start justify-between mb-3">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-gray-900">{{ number_format($tab === 'sent' ? $stats['sent_total'] : $stats['received_total']) }}</h3>
                    </div>
            ...
            <div class="bg-white rounded-xl p-6 border border-gray-200">
                <div class="flex items-start justify-between mb-3">
                    <div>
                        ...
                        <h3 class="text-3xl font-bold text-orange-600">{{ number_format($stats['received_unread']) }}</h3>
                    </div>
            ...
            <div class="divide-y divide-gray-200">
                ...
                @php
                    $icons = [
                        ...
                        'success' => ['icon' => 'star', 'color' => 'purple', 'bg' => 'purple-50', 'badge' => 'QUAN TRỌNG'],
                    ];
                ...
                <div class="notification-item p-6 hover:bg-gray-50 transition border-l-4 border-{{ $config['color'] }}-500 {{ (!$notification->is_read && $tab === 'received') ? 'bg-blue-50' : '' }} cursor-pointer"
                     ...
                     data-attachment="{{ $notification->attachment ? asset($notification->attachment->file_path) : '' }}"
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-4 mb-2">
                                <h3 class="{{ (!$notification->is_read && $tab === 'received') ? 'font-bold' : 'font-semibold' }} text-gray-900">{{ $notification->title }}</h3>
                                <span class="px-3 py-1 bg-{{ $config['color'] }}-100 text-{{ $config['color'] }}-700 rounded-full text-xs font-semibold whitespace-nowrap">
                                    {{ $config['badge'] }}
                                </span>
                            </div>
                            <p class="text-gray-600 text-sm mb-3 {{ (!$notification->is_read && $tab === 'received') ? 'font-medium' : '' }}">{{ $notification->message }}</p>
                            <div class="flex items-center gap-4 text-xs {{ (!$notification->is_read && $tab === 'received') ? 'text-gray-600 font-medium' : 'text-gray-500' }}">
                                ...
                                <span class="flex items-center gap-1">
                                    ...
                                    Đối tượng: {{ \App\Helpers\NotificationHelper::getRoleLabel($notification->target_role) }}
                                </span>
                                <span class="flex items-center gap-1">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            })" class="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center hover:bg-blue-100 transition-all border border-blue-100 shadow-sm" title="Chỉnh sửa">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button onclick="event.stopPropagation(); confirmDelete({{ $notification->id }})" 
                                    class="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center hover:bg-red-100 transition-all border border-red-100 shadow-sm" title="Xóa">
                ...
                <div class="p-12 text-center text-gray-500">
                    ...
                    @if(request('type') != '' && request('is_read') === '')
                        <p class="text-orange-600 font-medium">Vui lòng chọn thêm "Trạng thái" để hiển thị kết quả theo loại.</p>
    ...
    <div id="content-create" class="tab-content hidden">
        <div class="bg-white rounded-xl p-8 border border-gray-200">
            ...
            <form method="POST" action="{{ route('buyer.notifications.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="relative group">
                            <label id="dropZoneCreate" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50 hover:bg-blue-50 hover:border-blue-300 transition-all cursor-pointer">
                                <div id="uploadPlaceholderCreate" class="flex flex-col items-center justify-center pt-5 pb-6">
                                    <i class="fas fa-cloud-upload-alt text-3xl text-gray-400 group-hover:text-blue-500 mb-2"></i>
                                    <p class="text-sm text-gray-500"><span class="font-bold">Click để tải lên</span> hoặc kéo thả file</p>
                                    <p class="text-xs text-gray-400 mt-1">PDF (Tối đa 5MB)</p>
                                </div>
                                <div id="fileInfoCreate" class="hidden flex flex-col items-center justify-center pt-5 pb-6">
                                ...
                                <input type="file" name="attachment" id="attachmentCreate" accept=".pdf" class="hidden" onchange="handleFileSelect(this, 'Create')">
                            </label>
                        </div>
                    </div>
                </div>
...
<div id="notificationPageDetailModal" class="fixed inset-0 z-[10060] hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                {{-- Background overlay --}}
        <div class="fixed inset-0 transition-opacity bg-gray-900/80 backdrop-blur-md" onclick="closeNotificationDetail()" aria-hidden="true"></div>

        <div class="relative inline-block w-full max-w-7xl my-4 overflow-hidden text-left align-middle transition-all transform bg-white shadow-2xl rounded-3xl border border-white/20">
            {{-- Header Bar --}}
            <div class="absolute top-0 left-0 right-0 h-20 bg-white/80 backdrop-blur-md border-b border-gray-100 flex items-center justify-between px-8 z-20">
                <div class="flex items-center gap-4">
                    <div id="modalIconContainer_new" class="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600 shadow-sm">
                        <i class="fas fa-bell"></i>
                    </div>
                    <div>
                        <div id="modalBadge" class="inline-flex items-center px-2 py-0.5 rounded-lg text-[10px] font-bold tracking-widest uppercase mb-0.5 bg-blue-100 text-blue-700"></div>
                        <h2 id="modalTitle" class="text-xl font-bold text-gray-900 truncate max-w-md lg:max-w-2xl"></h2>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <a id="modalDownloadLink" href="#" target="_blank" class="w-10 h-10 rounded-xl bg-gray-50 text-gray-400 flex items-center justify-center hover:bg-gray-100 hover:text-blue-600 transition-all border border-gray-200 shadow-sm" title="Mở trong cửa sổ mới">
                        <i class="fas fa-external-link-alt"></i>
                    </a>
                    <div class="w-px h-6 bg-gray-200 mx-1"></div>
                    <button onclick="closeNotificationDetail()" class="w-10 h-10 rounded-xl bg-gray-900 text-white flex items-center justify-center hover:bg-gray-800 transition-all shadow-lg shadow-gray-200">
            ...
            <div class="flex flex-col lg:flex-row h-[90vh] pt-20">
                ...
                <div id="contentPane" class="w-full lg:w-[35%] border-r border-gray-100 bg-white overflow-y-auto custom-scrollbar transition-all duration-300">
                    <div class="p-8">
                        {{-- Metadata Section --}}
                        <div class="space-y-4 mb-8">
                            <div class="flex items-center justify-between p-4 bg-gray-50/50 rounded-2xl border border-gray-100/50 shadow-sm">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-white shadow-sm border border-gray-100 flex items-center justify-center text-blue-500">
                                        <i class="far fa-user text-lg"></i>
                                    </div>
                                    <div>
                                        <p class="text-[10px] uppercase font-black text-gray-400 tracking-widest mb-0.5">Người gửi</p>
                                        <p id="modalSender" class="font-bold text-gray-800"></p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-[10px] uppercase font-black text-gray-400 tracking-widest mb-0.5">Thời gian</p>
                                    <p id="modalTime" class="font-bold text-gray-800"></p>
                                </div>
                            </div>

                            <div class="flex items-center gap-3 p-4 bg-gray-50/50 rounded-2xl border border-gray-100/50 shadow-sm">
                                <div class="w-10 h-10 rounded-full bg-white shadow-sm border border-gray-100 flex items-center justify-center text-orange-500">
                                    <i class="far fa-building text-lg"></i>
                                </div>
                                <div>
                                    <p class="text-[10px] uppercase font-black text-gray-400 tracking-widest mb-0.5">Đối tượng nhận</p>
                        ...
                        <div class="relative">
                            ...
                            <h4 class="text-xs font-black text-blue-600 uppercase tracking-widest mb-4">Nội dung chi tiết</h4>
                            <div class="prose prose-sm prose-blue max-w-none">
                                <p id="modalMessage" class="text-gray-600 leading-relaxed text-base whitespace-pre-wrap font-medium"></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div id="pdfViewerArea" class="flex-1 bg-gray-100 relative overflow-hidden hidden transition-all duration-500">
                    ...
                    {{-- Empty State for PDF --}}
                    <div id="pdfPlaceholder" class="absolute inset-0 flex flex-col items-center justify-center text-gray-400 bg-gray-50 hidden">
                        <div class="w-24 h-24 rounded-3xl bg-gray-100 flex items-center justify-center mb-4 transition-transform hover:scale-110 duration-300">
                            <i class="fas fa-file-pdf text-4xl opacity-20"></i>
                        </div>
                        <p class="font-bold text-gray-400 uppercase tracking-widest text-xs">Không có tài liệu đính kèm</p>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white px-8 pt-8 pb-6">
                <div class="flex items-center justify-between mb-8">
                    <div class="flex items-center gap-4">
                        ...
                        <h3 class="text-2xl font-bold text-gray-900">Sửa thông báo</h3>
                    </div>
                ...
                <form id="editForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-gray-700 mb-2">Đối tượng nhận</label>
                                <select name="target_role" id="editTarget" required
                                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
                                    <option value="ALL">Tất cả người dùng</option>
                                    <option value="ADMIN">Quản trị viên</option>
                                    <option value="DEPARTMENT">Các khoa/Phòng</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-2">Thay đổi File PDF (Tùy chọn)</label>
                            <label class="relative flex items-center gap-3 px-4 py-3 border-2 border-dashed border-blue-100 rounded-xl bg-blue-50/50 hover:bg-blue-50 transition-all cursor-pointer group">
                                <div class="w-10 h-10 rounded-lg bg-white flex items-center justify-center text-blue-600 shadow-sm group-hover:scale-110 transition-transform">
                                    <i class="fas fa-file-upload"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p id="fileNameEdit" class="text-sm font-bold text-blue-700 truncate text-center">Chưa chọn file mới</p>
                                    <p class="text-[10px] text-blue-400 uppercase tracking-widest font-bold">Click để chọn file PDF</p>
                                </div>
                                <input type="file" name="attachment" id="attachmentEdit" accept=".pdf" class="hidden" onchange="handleFileSelect(this, 'Edit')">
                            </label>
                        </div>
                    </div>
...
function viewNotificationDetail(data) {
    ...
    const configs = {
        'success': { icon: 'fa-star', color: 'purple', badge: 'QUAN TRỌNG' },
...
function openEditModal(data) {
    ...
    const form = modal.querySelector('form'); // Select form within the modal
    
    // Set form action based on ID
    form.action = `/buyer/notifications/${data.id}`;
...
function confirmDelete(id) {
    if (confirm('Bạn có chắc chắn muốn xóa thông báo này? Hành động này không thể hoàn tác.')) {
        ...
        form.action = `/buyer/notifications/${id}`;
        form.submit();
    }
}
