...
@section('header_title', 'Cập nhật Nhà cung cấp')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('buyer.suppliers.update', $supplier->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-8 divide-y divide-gray-200">
                ...
                <div class="space-y-6 sm:space-y-5">
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                        ...
                        <div class="mt-1 sm:mt-0 sm:col-span-2">
                            <input type="text" name="supplier_code" id="supplier_code" value="{{ old('supplier_code', $supplier->supplier_code) }}" autocomplete="off" required readonly
                    ...
                    <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                        ...
                        <div class="mt-1 sm:mt-0 sm:col-span-2">
                            <textarea id="address" name="address" rows="3" class="max-w-lg shadow-sm block w-full focus:ring-blue-500 focus:border-blue-500 sm:text-sm border border-gray-300 rounded-md p-2">{{ old('address', $supplier->address) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="pt-8 space-y-6 sm:pt-10 sm:space-y-5">
                ...
                <div class="sm:border-t sm:border-gray-200 sm:pt-5">
                    <button type="button" onclick="document.getElementById('cat_container').classList.toggle('hidden'); document.getElementById('cat_arrow').classList.toggle('rotate-180')" 
                        ...
                        <span class="text-sm font-medium text-gray-700">Chọn danh mục cung cấp ({{ count($selectedCategories) }} đã chọn)</span>
                    ...
                    <fieldset id="cat_container" class="hidden mt-4">
                        <legend class="sr-only">Loại sản phẩm</legend>
                         <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                            ...
                            <div class="relative flex items-start p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors {{ in_array($category->id, $selectedCategories) ? 'bg-blue-50 border-blue-200' : '' }}">
            ...
            <div class="pt-8 space-y-6 sm:pt-10 sm:space-y-5">
                 <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start">
                    ...
                    <div class="mt-1 sm:mt-0 sm:col-span-2">
                        <textarea id="note" name="note" rows="3" class="max-w-lg shadow-sm block w-full focus:ring-blue-500 focus:border-blue-500 sm:text-sm border border-gray-300 rounded-md p-2">{{ old('note', $supplier->note) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
