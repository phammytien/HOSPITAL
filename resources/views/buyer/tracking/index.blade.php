...
@section('content')
            ...
            @foreach($tabs as $key => $tab)
                <a href="{{ route('buyer.tracking.index', array_merge(request()->query(), ['status' => $key])) }}"
                    ...
                    @if(isset($counts[$key]) && $counts[$key] > 0)
                        <span class="ml-2 px-2 py-0.5 rounded-full text-xs {{ $status === $key ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600' }}">
                            {{ $counts[$key] }}
                        </span>
                    @endif
                </a>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden relative">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition border-row">
                            ...
                            <td class="px-6 py-4 text-center">
                                @if($order->status === 'DELIVERED')
                                    <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-100 flex items-center justify-center gap-1 mx-auto w-fit">
                            ...
                            <td class="px-6 py-4 text-right text-sm text-gray-600">
                                ...
                                @endphp
                                {{ $deliveredItems }}/{{ $totalItems }} <span class="hidden sm:inline">sản phẩm về kho</span>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ !in_array($status, ['DELIVERED', 'COMPLETED', 'CANCELLED']) ? '7' : '6' }}" class="px-6 py-12 text-center text-gray-500 italic">
        ...
        @if(!in_array($status, ['DELIVERED', 'COMPLETED', 'CANCELLED']))
            ...
            <div id="bulk-action-bar" class="hidden fixed bottom-6 left-1/2 -translate-x-1/2 bg-white/95 backdrop-blur-sm shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-gray-100 rounded-2xl px-6 py-3 items-center gap-5 z-50 transition-all duration-300 transform animate-in fade-in slide-in-from-bottom-4">
                <div class="flex items-center gap-3">
                    <div class="flex items-center justify-center w-8 h-8 rounded-full bg-blue-50 text-blue-600 font-bold text-sm">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div class="px-5 py-2.5 bg-blue-50/50 text-blue-700 rounded-xl font-bold text-sm border border-blue-100 flex items-center gap-2 whitespace-nowrap">
                        <i class="fas fa-arrow-right text-[10px]"></i>
                        <span>{{ $targetLabel }}</span>
                    </div>
                    
                    <div id="bulk-date-container" class="{{ ($status === 'ORDERED') ? 'flex' : 'hidden' }} items-center gap-3 bg-blue-50/50 border border-blue-100 px-4 py-2 rounded-xl transition-all duration-300 whitespace-nowrap">
                        <div class="flex flex-col">
                            <label class="text-[10px] font-bold text-blue-600 uppercase tracking-wider mb-0.5 whitespace-nowrap">Ngày dự kiến giao</label>
                            <div class="flex items-center gap-2">
                                <i class="far fa-calendar-alt text-blue-500"></i>
                                <input type="date" name="expected_delivery_date" id="bulk-date-input" min="{{ date('Y-m-d') }}"
                    ...
                    <button type="button" id="submit-bulk-btn" class="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-700 hover:shadow-lg transition-all duration-300 flex items-center gap-2 group whitespace-nowrap shadow-sm">
