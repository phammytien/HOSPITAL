...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
            <form action="{{ route('admin.dashboard') }}" method="GET"
                class="flex items-center gap-2 bg-white rounded-lg p-1.5 shadow-sm border border-gray-100 w-fit">
                ...
                <!-- Year Select -->
                <select name="year"
                    class="bg-gray-50 border-0 rounded-md text-sm font-medium text-gray-700 py-1.5 pl-3 pr-8 focus:ring-0 focus:bg-gray-100 cursor-pointer hover:bg-gray-100 transition-colors"
                ...
                <select name="department_id"
                    ...
                    @foreach($departments as $dept)
                        <option value="{{ $dept->id }}" {{ $deptId == $dept->id ? 'selected' : '' }}>{{ $dept->department_name }}
                        </option>
                    @endforeach
                </select>
            </form>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div
                ...
                <div class="flex justify-between items-start mb-3">
                    ...
                    <div
                        class="w-10 h-10 rounded-lg bg-orange-50 flex items-center justify-center text-orange-500 group-hover:bg-orange-100 transition-colors">
            ...
            <div
                ...
                <div class="flex justify-between items-start mb-3">
                    <div>
                        <p class="text-xs font-bold text-gray-400 uppercase tracking-wider">Đã duyệt
                            ({{ $periodType == 'year' ? 'Năm' : ($periodType == 'quarter' ? 'Quý' : 'Tháng') }})</p>
                        <h3 class="text-3xl font-extrabold text-gray-800 mt-1">{{ $approvedThisPeriod }}</h3>
                    </div>
                    <div
                        class="w-10 h-10 rounded-lg bg-green-50 flex items-center justify-center text-green-500 group-hover:bg-green-100 transition-colors">
            ...
            <div
                ...
                <div class="flex justify-between items-start mb-3">
                    ...
                    <div
                        class="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center text-blue-500 group-hover:bg-blue-100 transition-colors">
            ...
            <div
                ...
                <div class="flex justify-between items-start mb-3">
                    ...
                    <div
                        class="w-10 h-10 rounded-lg bg-purple-50 flex items-center justify-center text-purple-500 group-hover:bg-purple-100 transition-colors">
                        ...
                        <div class="flex items-center justify-between text-sm">
                            <div class="flex items-center">
                                <span class="w-2.5 h-2.5 rounded-full mr-2"
                                    style="background-color: {{ $colors[$index % 5] }}"></span>
                                <span class="text-gray-600 truncate max-w-[140px]">{{ $label }}</span>
                            </div>
                            <span
                                class="font-bold text-gray-800">{{ number_format($chartData['categories']['data'][$index] ?? 0) }}đ</span>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col">
            <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                ...
                <a href="{{ route('admin.history') }}"
                    class="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center group">
            ...
            <div class="overflow-x-auto flex-grow p-2">
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($recentRequests as $request)
                            <tr class="hover:bg-gray-50 transition-colors group">
                                ...
                                <td class="px-4 py-4 text-gray-700 font-medium">
                                    {{ $request->department->department_name ?? '-' }}</td>
                                ...
                                <td class="px-4 py-4">
                                    ...
                                    @else
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-gray-100 text-gray-700">{{ $request->status }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <button onclick="viewOrderDetail({{ $request->id }})"
                                        class="text-gray-400 hover:text-blue-600 transition-colors" title="Xem chi tiết">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-8 text-center text-gray-400">Chưa có dữ liệu giao dịch</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="space-y-4">
                @forelse($recentActivities->take(4) as $activity)
                    <div class="flex items-start gap-3 relative pb-4 border-l-2 border-gray-100 pl-4 last:pb-0 last:border-0">
                        <div
                            class="absolute -left-[5px] top-1.5 w-2 h-2 rounded-full bg-{{ $activity['icon_color'] ?? 'blue' }}-500 ring-4 ring-white">
                        </div>
                        <div>
                ...
                <a href="{{ route('admin.notifications') }}"
                    class="block w-full text-center text-xs text-blue-600 font-bold hover:bg-blue-50 py-2 rounded-lg transition-colors mt-2">Xem
                    tất cả <i class="bi bi-arrow-right ml-1"></i></a>
            </div>
        </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                <div class="space-y-3">
                    @forelse($topDepartments as $index => $dept)
                        <div
                            class="flex items-center justify-between p-3 hover:bg-gray-50 rounded-lg transition-colors border border-gray-100">
                            <div class="flex items-center gap-3 flex-1">
                                <div
                                    class="w-10 h-10 bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg flex items-center justify-center flex-shrink-0 text-white font-bold text-sm">
                                    #{{ $index + 1 }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-bold text-gray-800 truncate">
                                        {{ $dept->department->department_name ?? 'N/A' }}</p>
                            ...
                            <div class="flex items-center gap-2">
                                <div class="w-16 bg-gray-100 rounded-full h-2">
                                    ...
                                    @endphp
                                    <div class="bg-blue-600 h-2 rounded-full transition-all" style="width: {{ $percent }}%">
                                    </div>
                                </div>
    ...
    <div id="orderDetailModal"
        class="fixed inset-0 bg-black bg-opacity-50 hidden z-[60] flex items-center justify-center p-4 transition-opacity duration-300">
        <div
            class="bg-gray-50 rounded-xl shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto transform transition-all scale-100">
            <!-- Header -->
            <div
                class="sticky top-0 bg-white border-b border-gray-200 px-8 py-5 flex justify-between items-center z-10 shadow-sm">
                ...
                <!-- <div class="flex space-x-3">
                    ...
                    <button
                        class="flex items-center px-4 py-2 border border-transparent rounded-lg text-white bg-blue-600 hover:bg-blue-700 font-medium text-sm transition-colors shadow-sm">
