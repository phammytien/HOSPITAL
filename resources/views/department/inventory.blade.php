...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Quản lý kho phòng ban</h2>
            <p class="text-gray-600 mt-1">Theo dõi tồn kho của {{ Auth::user()->department->department_name ?? 'phòng ban' }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-2">
                <span class="text-sm text-gray-600">Kho của phòng</span>
            ...
            <p class="text-xs text-gray-500 mt-2">Kho đang quản lý</p>
        </div>
    ...
    <!-- Filters and Search -->
    <form method="GET" action="{{ route('department.inventory') }}" id="filterForm" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Kho</label>
                <select name="warehouse_id" id="warehouseFilter" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div>
                ...
                <select name="category_id" id="categoryFilter" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="text" name="search" id="searchInput" value="{{ request('search') }}" placeholder="Tên sản phẩm, mã SKU..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        ...
        <div class="mt-4 flex justify-end space-x-2">
            ...
            <a href="{{ route('department.inventory') }}" class="border border-gray-300 bg-white hover:bg-gray-50 text-gray-600 px-4 py-2 rounded-lg">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-3">
                                    <p class="font-medium text-gray-900">{{ $item->warehouse->warehouse_name }}</p>
                        ...
                        <td class="px-6 py-4">
                            <span class="text-lg font-bold {{ $item->quantity < 10 ? 'text-red-600' : ($item->quantity < 50 ? 'text-orange-600' : 'text-green-600') }}">
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                            ...
                            <p class="text-sm mt-2">Phòng ban chưa có kho hoặc chưa có sản phẩm nào</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="px-6 py-4 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div class="flex space-x-2">
                    ...
                    @else
                        <a href="{{ $inventory->previousPageUrl() }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
                            <i class="fas fa-chevron-left mr-1"></i>Trước
                        </a>
                    @endif
                    ...
                    @if ($inventory->hasMorePages())
                        <a href="{{ $inventory->nextPageUrl() }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
